...
}, { passive: true });\n(function(){const r=document.getElementById("lumiBot"),b=document.getElementById("lumiBotButton"),q=document.getElementById("lumiBubble");if(!r||!b||!q||!("speechSynthesis"in window))return;let v=null,s=0,n=-1;function voices(){const a=speechSynthesis.getVoices();v=a.find(x=>/^id(-|_)/i.test(x.lang))||a.find(x=>/^ms(-|_)/i.test(x.lang))||a[0]||null}function first(){const h=new Date().getHours(),g=h<11?"pagi":h<15?"siang":h<18?"sore":"malam",t=h<18?"Hari ini":"Malam ini";return "Selamat "+g+". Aku Lumi Bot. Aku diciptakan oleh tuanku, Diky. "+t+", aku akan menemanimu membaca sesuatu yang dia titipkan. Kalau kamu siap... mari kita mulai."}const a=[first(),"Baik... sekarang, biarkan kata-kata Diky berbicara sendiri.","Aku akan tetap di sini. Nikmati ceritanya.","Aku mulai mengerti kenapa cerita ini dibuat.","Tidak semua perasaan mudah dijelaskan.","Jangan terburu-buru. Ada kata-kata yang lebih baik dibaca perlahan.","Aku memang cuma Lumi Bot, tapi aku tahu bagian ini berarti.","Sepertinya Diky sudah mengatakan banyak hal.","Tinggal sedikit lagi. Jangan berhenti di sini.","Oh... kamu menemukan aku.","Tenang. Aku tidak akan membocorkan ending-nya."];voices();speechSynthesis.onvoiceschanged=voices;b.onclick=()=>{if(s){speechSynthesis.cancel();s=0;r.classList.remove("is-speaking");return}let i=Math.floor(Math.random()*a.length);if(i===n)i=(i+1)%a.length;n=i;const u=new SpeechSynthesisUtterance(a[i]);if(v)u.voice=v;u.lang=v?.lang||"id-ID";u.rate=.92;u.pitch=1.12;u.volume=.82;q.textContent=a[i];s=1;r.classList.add("is-speaking");u.onend=u.onerror=()=>{s=0;r.classList.remove("is-speaking")};speechSynthesis.speak(u)}})();