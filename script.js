...
let lumiReady = false;
let lumiSpeaking = false;
let lumiSpeechToken = 0;
let lumiBubbleTimer = null;
let lumiClickCount = 0;
let lumiLastClick = 0;
let lumiVoices = [];
const chapterLabels = [
 "THE FIRST PAGE",
...
function openLetter() {
 ...
 lumiSpeak("Baik... Sekarang, biarkan kata-kata Diky berbicara sendiri. Aku akan tetap di sini.", {bubble:true, mood:"happy"});
 setTimeout(() => {
 goToScene(2);
 }, 850);
}
...
/* Lumi Bot runtime — isolated from existing site logic */
function setupLumiBot(){
 const bot=document.getElementById("lumiBot");
 const bubble=document.getElementById("lumiBubble");
 if(!bot||!bubble)return;
 lumiReady=true;
 const initVoices=()=>{lumiVoices=window.speechSynthesis?window.speechSynthesis.getVoices():[];};
 initVoices();
 if(window.speechSynthesis) window.speechSynthesis.onvoiceschanged=initVoices;
 bot.addEventListener("click",()=>{
  const now=Date.now();
  if(lumiSpeaking){ stopLumiSpeech(); return; }
  if(now-lumiLastClick>2600) lumiClickCount=0;
  lumiLastClick=now; lumiClickCount++;
  const lines=[
   "Oh... kamu menemukan aku.",
   "Aku kira aku cuma akan menjadi pajangan.",
   "Baiklah... aku akan mengaku. Aku juga penasaran bagaimana akhir cerita ini.",
   "Tenang. Aku tidak akan membocorkan ending-nya."
  ];
  lumiSpeak(lines[Math.min(lumiClickCount,4)-1],{bubble:true,mood:lumiClickCount===3?"think":"happy"});
 });
 bot.addEventListener("keydown",e=>{if(e.key==="Enter"||e.key===" "){e.preventDefault();bot.click();}});
 // Try the opening greeting once. Mobile browsers may block speech until a user gesture.
 setTimeout(()=>{if(document.visibilityState==="visible") lumiSpeak(lumiGreeting(),{bubble:true,mood:"happy",opening:true});},900);
}
function lumiGreeting(){
 const h=new Date().getHours();
 if(h>=5&&h<11)return "Selamat pagi. Aku Lumi Bot. Aku diciptakan oleh tuanku, Diky. Hari ini, aku akan menemanimu membaca sesuatu yang dia titipkan. Kalau kamu siap... mari kita mulai.";
 if(h>=11&&h<15)return "Selamat siang. Aku Lumi Bot. Aku diciptakan oleh tuanku, Diky. Hari ini, aku akan menemanimu membaca sesuatu yang dia titipkan. Kalau kamu siap... mari kita mulai.";
 if(h>=15&&h<18)return "Selamat sore. Aku Lumi Bot. Aku diciptakan oleh tuanku, Diky. Sore ini, aku akan menemanimu membaca sesuatu yang dia titipkan. Kalau kamu siap... mari kita mulai.";
 return "Selamat malam. Aku Lumi Bot. Aku diciptakan oleh tuanku, Diky. Malam ini, aku akan menemanimu membaca sesuatu yang dia titipkan. Kalau kamu siap... mari kita mulai.";
}
function lumiPickVoice(){
 const voices=lumiVoices.length?lumiVoices:(window.speechSynthesis?window.speechSynthesis.getVoices():[]);
 if(!voices.length)return null;
 const id=voices.filter(v=>/^id(-|_)/i.test(v.lang));
 const female=/female|woman|perempuan|zira|siti|google indonesia/i;
 return id.find(v=>female.test((v.name||"")+" "+(v.voiceURI||"")))||id.find(v=>/google|gadis|indonesia/i.test(v.name||""))||id[0]||voices.find(v=>/^en/i.test(v.lang))||voices[0];
}
function lumiSpeak(text,opt={}){
 if(!lumiReady||!window.speechSynthesis)return false;
 const token=++lumiSpeechToken;
 try{window.speechSynthesis.cancel();}catch(_){ }
 const u=new SpeechSynthesisUtterance(text);
 const v=lumiPickVoice();
 if(v)u.voice=v;
 u.lang=v&&/^id/i.test(v.lang)?v.lang:"id-ID";
 u.rate=.86; u.pitch=1.12; u.volume=.82;
 u.onstart=()=>{if(token!==lumiSpeechToken)return; lumiSpeaking=true; lumiSetSpeaking(true); if(opt.bubble)lumiShowBubble(text);};
 u.onend=()=>{if(token!==lumiSpeechToken)return; lumiSpeaking=false; lumiSetSpeaking(false);};
 u.onerror=()=>{if(token!==lumiSpeechToken)return; lumiSpeaking=false; lumiSetSpeaking(false); if(opt.bubble)lumiShowBubble(text);};
 try{window.speechSynthesis.speak(u);return true;}catch(_){return false;}
}
function stopLumiSpeech(){lumiSpeechToken++; if(window.speechSynthesis)try{window.speechSynthesis.cancel();}catch(_){ } lumiSpeaking=false;lumiSetSpeaking(false);}
function lumiSetSpeaking(on){const bot=document.getElementById("lumiBot");if(bot)bot.classList.toggle("lumi-speaking",!!on);}
function lumiShowBubble(text){const b=document.getElementById("lumiBubble");if(!b)return;b.textContent=text;b.classList.add("show");clearTimeout(lumiBubbleTimer);lumiBubbleTimer=setTimeout(()=>b.classList.remove("show"),Math.min(7000,Math.max(2800,text.length*42)));}
const lumiOriginalVisibilityHandler=document.visibilitychange;
document.addEventListener("visibilitychange",()=>{if(document.visibilityState!=="visible")stopLumiSpeech();});
