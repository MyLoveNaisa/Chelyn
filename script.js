...
document.addEventListener("visibilitychange", () => {
    if (!particles) return;
    particles.style.display = document.visibilityState === "visible" ? "" : "none";
});

if (bgm) bgm.volume = 0.42;
if (paperSound) paperSound.volume = 0.72;

/* FINAL STABILITY + LIGHTWEIGHT POLISH */
(function () {
    const scenes = document.querySelectorAll('.scene');
    scenes.forEach(scene => {
        scene.addEventListener('touchmove', event => {
            // Chapters are intentionally horizontal-only. This prevents Android
            // overscroll/bounce from making the fixed scene appear to move.
            if (Math.abs((event.touches[0]?.clientY || 0) - (scene._touchStartY || 0)) > 8) {
                event.preventDefault();
            }
        }, { passive:false });
        scene.addEventListener('touchstart', event => {
            scene._touchStartY = event.touches[0]?.clientY || 0;
        }, { passive:true });
    });
