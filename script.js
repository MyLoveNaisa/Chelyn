...
/* ULTRA MAX SMOOTH — startup/resume guard */
(() => {
  let resumeFrame = 0;
  let resumeTimer = 0;
  ...
  const smoothResume = () => {
    ...
    root.classList.add("ultra-resuming");
    resumeFrame = requestAnimationFrame(() => {
      resumeTimer = setTimeout(() => {
        root.classList.remove("ultra-resuming");
      }, 120);
    });
  };

  document.addEventListener("visibilitychange", () => {
    if (!document.hidden) smoothResume();
  }, { passive: true });

  window.addEventListener("pageshow", smoothResume, { passive: true });
})();
